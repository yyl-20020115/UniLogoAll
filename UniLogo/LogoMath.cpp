...
//#define errchk(x) x
#define errchk(x) { errno = 0; x; if (errno) err_logo(ERR_TYPES::BAD_DATA_UNREC,arg); }/*ignore*/
            ...
            if (!imode)
            {
                ...
                if (setjmp(oflo_buf) == 0)
                {
                    ...
                    default:  // logical op
                        if (nodetype(arg) == INTEGER)
                        {
                            err_logo(ERR_TYPES::BAD_DATA_UNREC, make_floatnode(fval));
                        }
                        else
                        {
                            err_logo(ERR_TYPES::BAD_DATA_UNREC, arg);
...
static
long compare_numnodes(NODE *n1, NODE *n2)
{
...
//    0, if nd1 = nd2
//  > 0, if nd1 > nd2
long compare_node(NODE *n1, NODE *n2, bool ignorecase)
{
    long icmp = 0;
...
NODE *lbeforep(NODE *args)
{
    ...
    bool caseig = isCaseIgnored();
    long val    = compare_node(arg1, arg2, caseig);
