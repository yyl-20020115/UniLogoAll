...
bool CNetworkConnection::SendValue(const char * Data)
{
    ...
    {
		ws->Write(Data, (unsigned int)strlen(Data) + 1);
        // send the data
...
NODE *lnetstartup(NODE *args)
{
    ...
    if (args != NIL)
    {
        network_dns_sync = (int)int_arg(args);
    }
...
NODE *lnetaccepton(NODE *args)
{
    ...
    FIXNUM port = getint(nonnegative_int_arg(args));

	wxString networksend = ArgumentUtils::cnv_strnode_string(cdr(args));

    wxString networkreceive = ArgumentUtils::cnv_strnode_string(cdr(cdr(args)));

    if (NOT_THROWING)
    {
        g_ServerConnection.Enable(networksend, networkreceive,(int) port);
    }
...
NODE *lnetconnecton(NODE *args)
{
    ...
    FIXNUM remote_port = getint(nonnegative_int_arg(cdr(args)));

	wxString networksend = ArgumentUtils::cnv_strnode_string(cdr(cdr(args)));

	wxString networkreceive= ArgumentUtils::cnv_strnode_string(cdr(cdr(cdr(args))));
