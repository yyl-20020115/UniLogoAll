...
void create_local_variable(NODE * caseobject, NODE * var_stack_position)
{
    ...
    if (flag__caseobj(caseobject, VAL_STEPPED))
    {
        ...
        err_logo(ERR_TYPES::SHADOW_WARN, caseobject);
    }
}
...
void trace_input(NODE * UnquotedArgument)
{
    ...
    print_node(GetOutputStream(), MESSAGETYPE::Trace, quoted_arg);
    deref(quoted_arg);
...
static
NODE *evaluator(NODE *list, labels where)
{
...
begin_line:
    ...
    Stack.PushFrame(labels::end_line);
    goto begin_seq;
...
begin_seq:
 ...
 end_line:
    ...
    if (val != Unbound)
    {
        if (NOT_THROWING)
        {
            err_logo(ERR_TYPES::DK_WHAT, val);
        }
        deref(val);
    }
    val = NIL;
 ...
 eval_dispatch:
    ...
    case COLON:
        ...
        {
            assign(val, err_logo(ERR_TYPES::NO_VALUE, node__colon(exp)));
        }
        goto fetch_cont;
 ...
 eval_arg_loop:
    ...
    if (exp == g_ParseErrorNotEnoughInputs)
    {
        if (NOT_THROWING)
        {
            err_logo(ERR_TYPES::NOT_ENOUGH, NIL);
        }
        goto eval_args_done;
    }

    Stack.PushFrame(
        labels::accumulate_arg,
        argl,
        qm_list,
        unev,
        fun,
 ...
 accumulate_arg:
    ...
    {
        assign(val, err_logo(ERR_TYPES::DIDNT_OUTPUT, NIL));
    }
 ...
 eval_args_done:
    ...
    assert(unev == NIL || stopping_flag == CTRLTYPE::THROWING);
 ...
 apply_dispatch:
    ...
    if (proc == UNDEFINED)
    {
        ...
        if (NOT_THROWING)
        {
            assign(val, err_logo(ERR_TYPES::DK_HOW, fun));
        }
        else
        {
            assign(val, Unbound);
 ...
 goto_cont:
    ...
    switch (where)
    {
        case labels::all_done: goto all_done;
        case labels::begin_line: goto begin_line;
        case labels::end_line: goto end_line;
        case labels::begin_seq: goto begin_seq;
        case labels::begin_apply: goto begin_apply;
        case labels::eval_sequence_continue: goto eval_sequence_continue;
        case labels::accumulate_arg: goto accumulate_arg;
        case labels::compound_apply_continue: goto compound_apply_continue;
        case labels::set_args_continue: goto set_args_continue;
        case labels::macro_return: goto macro_return;
        case labels::after_lambda: goto after_lambda;
        case labels::runresult_continuation: goto runresult_continuation;
        case labels::runresult_followup: goto runresult_followup;
        case labels::repeat_continuation: goto repeat_continuation;
        case labels::repeat_followup: goto repeat_followup;
        case labels::catch_continuation: goto catch_continuation;
        case labels::catch_followup: goto catch_followup;
        case labels::goto_continuation: goto goto_continuation;
        case labels::NUM_TOKENS:
        default :
            abort();
    }
 ...
 compound_apply:
    ...
    if ((tracing = ((flag__caseobj(fun, PROC_TRACED) || traceflag))))
    {
        ...
        trace_level++;
        ndprintf(GetOutputStream(), MESSAGETYPE::Trace, L"( %s ", fun);
    }
 ...
 lambda_apply:
         ...
         formals = cdr(formals))
    {
        ...
        if (nodetype(parm) == CASEOBJ)
        {
            ...
            if (arg == Unbound)
            {
                ...
                err_logo(ERR_TYPES::NOT_ENOUGH, fun);
    ...
    if (argl != NIL) 
    {
        ...
        //
        err_logo(ERR_TYPES::TOO_MUCH, fun);
    }
    if (check_throwing)
    {
        assign(val, Unbound);
    ...
    if ((tracing = ((!is_list(fun) && flag__caseobj(fun, PROC_TRACED)) || traceflag)))
    {
        if (NOT_THROWING) 
        {
            print_char(GetOutputStream(),MESSAGETYPE::Trace, ')');
        }
        new_line(GetOutputStream(), MESSAGETYPE::Trace);

        Stack.PushFrame(
            labels::compound_apply_continue,
            fun);
    }
    assign(val, Unbound);
...
eval_sequence: 
 ...
 no_fixup_necessary:
        ...
        if ((ufun != NIL && flag__caseobj(ufun, PROC_STEPPED)) || stepflag)
        {
            ...
            if (tracing || traceflag)
            {
                ...
                while (i++ < trace_level) 
                {
                    print_space(stdoutstream, MESSAGETYPE::Trace);
                }
            }
        ...
        is_tailform(procnode__caseobj(car(exp))))
    {
        ...
        if (expression_priority == OUTPUT_PRIORITY)
        {
            ...
            if (cadr(exp) == g_ParseErrorNotEnoughInputs)
            {
                ...
                err_logo(ERR_TYPES::NOT_ENOUGH, caseobj);
                assign(val, Unbound);
                goto fetch_cont;
            }
            ...
            else if (ufun == NIL)
            {
                ...
                err_logo(ERR_TYPES::AT_TOPLEVEL, caseobj);
                assign(val, Unbound);
                goto fetch_cont;
            }
 ...
 eval_sequence_continue:
    ...
    if (stopping_flag == CTRLTYPE::MACRO_RETURN)
    {
        if (unev == Unbound) 
        {
            assign(unev, NIL);
    ...
    else if (!IsInMacro(g_ValueStatus))
    {
        ...
        if (stopping_flag == CTRLTYPE::OUTPUT || STOPPING)
        {
            stopping_flag = CTRLTYPE::RUN;
            ...
            if (NOT_THROWING)
            {
                if (val != Unbound)
                {
                        ...
                        g_ValueStatus == VALUE_STATUS_Required) // BUG???
                    {
                        ...
                        err_logo(ERR_TYPES::DIDNT_OUTPUT, Output);
                    }
                }
                else 
                {
    ...
    if (NOT_THROWING && val != Unbound)
    {
        ...
        err_logo((unev == NIL ?ERR_TYPES:: DK_WHAT_UP : ERR_TYPES::DK_WHAT), val);
        assign(val, Unbound);
    }
    if (NOT_THROWING && (unev == NIL || unev == Unbound))
    {
        ...
        if (g_ValueStatus == VALUE_STATUS_Required) 
        {
            err_logo(ERR_TYPES::DIDNT_OUTPUT, NIL);
        }
        goto fetch_cont;
    }
 ...
 compound_apply_continue:
    ...
    if (NOT_THROWING)
    {
        ...
        print_node(GetOutputStream(), MESSAGETYPE::Trace, fun);
        if (val == Unbound)
        {
 ...
 macro_return:
    ...
    {
        assign(val, err_logo(ERR_TYPES::ERR_MACRO, val));
    }
    if (NOT_THROWING)
    {
        if (is_cont(val))
        {
            ...
            where = static_cast<labels>(cont__cont(val));
        ...
        else if (tailcall == 0)
        {
            ...
            if (stopping_flag != CTRLTYPE::THROWING)
            {
                stopping_flag = CTRLTYPE::MACRO_RETURN;
            }
            goto fetch_cont;
        }
        else
 ...
 runresult_continuation:
    assign(list, val);
    Stack.PushFrame(labels::runresult_followup);
    g_ValueStatus = VALUE_STATUS_ValueMaybeOkInMacro;
    goto begin_seq;
 ...
 catch_followup:
        ...
        if (stopping_flag == CTRLTYPE::OUTPUT || STOPPING)
        {
            ...
            if (val != Unbound)
            {
                if (g_ValueStatus == VALUE_STATUS_NotOk ||
                    g_ValueStatus == VALUE_STATUS_Required) // BUG???
                {
                    err_logo(ERR_TYPES::DK_WHAT_UP, val);
                }
            }
        }
    }
    ...
    if (stopping_flag == CTRLTYPE::THROWING)
    {
        ...
        // version of UniLogo.  Therefore, we must treat them as 
 ...
 goto_continuation:
    ...
    if (ufun == NIL)
    {
        err_logo(ERR_TYPES::AT_TOPLEVEL, Goto);
        assign(val, Unbound);
        goto fetch_cont;
    }
    ...
    err_logo(ERR_TYPES::BAD_DATA_UNREC, val);
    assign(val, Unbound);
    goto fetch_cont;
 ...
 begin_apply:
    ...
    {
        assign(fun, err_logo(ERR_TYPES::APPLY_BAD_DATA, fun));
    }
    assign(argl, cadr(val));
    assign(val, Unbound);
    while (!is_list(argl) && NOT_THROWING)
    {
        assign(argl, err_logo(ERR_TYPES::APPLY_BAD_DATA, argl));
    }
    if (NOT_THROWING && fun != NIL)
    {
        if (is_list(fun))
        {
            ...
            if (is_list(car(fun)) && cdr(fun) != NIL)
            {
                ...
                else
                {
                    ...
                    {
                        // too many inputs.
                        err_logo(ERR_TYPES::TOO_MUCH, car(argl));
                        goto fetch_cont;
                    }
                    else
                    {
        ...
        else
        {
            ...
            {
                assign(val, err_logo(ERR_TYPES::DK_HOW_UNREC, fun));
            }
            if (NOT_THROWING)
            {
                FIXNUM min=0;
                ...
                else
                {
                    ...
                    if (getprimdflt(proc) < 0)
                    {
                        ...
                        err_logo(ERR_TYPES::DK_HOW_UNREC, fun); // can't apply
                        goto fetch_cont;
                    }
                    else
                    {
                ...
                {
                    err_logo(ERR_TYPES::TOO_MUCH, NIL);
                }
                else
                {
                    goto apply_dispatch;
...
void eval_driver(NODE *line)
{
    if (line != NIL)
    {
        evaluator(line,labels:: begin_line);
    }
}
...
{
    g_ValueStatus = VALUE_STATUS_ValueMaybeOkInMacro;
    return evaluator(seq,labels:: begin_seq);
}
...
{
    return make_cont(labels::begin_apply, args);
}
...
NODE *lqm(NODE *args)
{
    ...
    if (np == NIL)
    {
        return err_logo(ERR_TYPES::BAD_DATA_UNREC, make_intnode(argnum));
    }
    return car(np);
}
...
NODE *llocal(NODE *args)
{
    ...
    {
        setcar(args, err_logo(ERR_TYPES::BAD_DATA, car(args)));
    }
    ...
    while (args != NIL && NOT_THROWING)
    {
        ...
        {
            arg = err_logo(ERR_TYPES::BAD_DATA, arg);
...
bool process_special_conditions()
{
    ...
    if (stopping_flag == CTRLTYPE::THROWING)
    {
        ...
        {
            err_logo(ERR_TYPES::NO_CATCH_TAG, throw_node);
            err_print();
            error_happened = true;
        }
    ...
    if (stopping_flag == CTRLTYPE::STOP || stopping_flag == CTRLTYPE::OUTPUT)
    {
...
void start_execution()
{
    ...
    // Enable the Halt/Pause buttons since UniLogo is running.
...
void stop_execution()
{
    ...
    // Disable the Halt/Pause buttons since UniLogo is not running.
    UpdateUiExecutionState();
    ...
    if (halt_flag == 0 && IsTimeToExit)
    {
        ...
        // Don't let any errors that were thrown stop UniLogo from exiting.
        stopping_flag = CTRLTYPE::RUN;
