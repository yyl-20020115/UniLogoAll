...
static NODE *firstable_arg(NODE *args)
{
    NODE *arg = car(args);
...
NODE *memberp_helper(NODE *args, bool notp, bool substr)
{
    ...
    else if (nodetype(obj2) == ARRAY)
    {
        ...
        if (notp)
        {
            err_logo(ERR_TYPES::BAD_DATA_UNREC, obj2);
        }
...
NODE *setitem_helper(NODE *args, bool safe)
{
    ...
    if (NOT_THROWING)
    {
        ...
        if (safe)
        {
            ...
            {
                setcar(cddr(args), err_logo(ERR_TYPES::BAD_DATA, cont));
                cont = car(cddr(args));
            }
        }
        if (NOT_THROWING)
...
NODE *lform(NODE *args)
{
    ...
    if (width < 0)
    {
        NODE * error_node = err_logo(ERR_TYPES::BAD_DATA, car(width_arg));
...
NODE *l_setfirst(NODE *args)
{
    ...
    {
        setcar(args, err_logo(ERR_TYPES::BAD_DATA, list));
        list = car(args);
    }
