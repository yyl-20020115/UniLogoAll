...
// used for its KEYBOARDON events, and for compatibility is what UniLogo also uses.
// On Windows, these are defined with as VK_*, but since UniLogo can be compiled
...
// Key Code, which is what UniLogo programs expect to be given.
...
void CScreen::OnKeyDown(wxKeyEvent& Event)
{
    ...
    if (KeyboardCapture == KEYBOARDCAPTURE::Off)
    {
        if (keyCode == WXK_TAB)
        {
    ...
    if (KeyboardCapture == KEYBOARDCAPTURE::KeyDownKeyUp)
    {
    ...
    else if (KeyboardCapture == KEYBOARDCAPTURE::Off &&
...
void CScreen::OnChar(wxKeyEvent& Event)
{
    ...
    if (KeyboardCapture == KEYBOARDCAPTURE::KeyDown)
    {
