...
static NODE *make_procnode(NODE *lst, NODE *wrds, short min, short df, short max)
{
    return cons_list(
        lst, 
        wrds,
...
static NODE *get_bodywords(NODE *proc, NODE *name)
{
...
static NODE *name_arg(NODE *args)
{
...
// Throws recoverable bad input errors until it gets one.
static NODE *proc_name_arg(NODE *args)
{
...
NODE *ltext(NODE *args)
{
    ...
    if (NOT_THROWING)
    {
        ...
        if (val == UNDEFINED)
        {
            err_logo(ERR_TYPES::DK_HOW_UNREC, name);
            return Unbound;
        }
        else if (is_prim(val))
        {
            err_logo(ERR_TYPES::IS_PRIM, name);
            return Unbound;
        }
        else
        {
...
static void set_new_generation()
{
    assign(the_generation, cons_list(NIL));
}

static bool is_list_of_lists(const NODE *val)
{
    if (val == NIL) 
    {
...
static int get_default_args_for_procedure(const NODE * procedure)
{
    return is_prim(procedure) ? 
        getprimdflt(procedure) :
        (int)getint(dfltargs__procnode(procedure));
}

static NODE *define_helper(NODE *args, int macro_flag)
{
    ...
    if (macro_flag >= 0)
    {
        ...
        if (NOT_THROWING)
        {
            ...
            while ((val == NIL || !is_list_of_lists(val)) &&
                   NOT_THROWING)
            {
                setcar(cdr(args), err_logo(ERR_TYPES::BAD_DATA, val));
                val = cadr(args);
            }
        }
    }
    ...
    if (NOT_THROWING)
    {
        ...
        while (args != NIL)
        {
            ...
            {
                err_logo(ERR_TYPES::BAD_DATA_UNREC, arg);
                break;
            }
...
static NODE *to_helper(NODE *args, bool is_macro)
{
    if (ufun != NIL && *GetLoadStream() == stdin)
    {
        err_logo(ERR_TYPES::NOT_INSIDE, NIL);
        return Unbound;
    }
	...
	GetInputMode() = INPUTMODE::INPUTMODE_To;
    ...
    else
    {
        ...
        if (old_proc != UNDEFINED)
        {
            ...
            old_default = (unsigned int)getint(dfltargs__procnode(old_proc));
        }
    ...
    if (NOT_THROWING)
    {
        ...
        if (to_pending && NOT_THROWING)
        {
            ...
            // Set the "dirty" flag so that closing UniLogo will
...
static NODE * get_contents(CNTLSTTYP contents_type)
{
    CAppendableList contents_list;
...
//
static NODE *get_contents_list(int filter)
{
    want_buried = filter;
    ...
    ret = cons(get_contents(CNTLSTTYP::c_PLISTS), ret);

    // Get all variables that match the filter
    // and prepend them to the contents list
    ret = cons(get_contents(CNTLSTTYP::c_VARS), ret);

    // Get all procedures that match the filter
    // and prepend them to the contents list
    ret = cons(get_contents(CNTLSTTYP::c_PROCS), ret);
...
NODE *lprocedures(NODE *)
{
    ...
    return get_contents(CNTLSTTYP::c_PROCS);
}
...
NODE *lnames(NODE *)
{
    ...
    return cons_list(NIL, get_contents(CNTLSTTYP::c_VARS));
}
...
NODE *lplists(NODE *)
{
    ...
    return cons_list(NIL, NIL, get_contents(CNTLSTTYP::c_PLISTS));
}

static NODE *one_list(NODE *nd)
{
    if (!is_list(nd))
    {
        return cons_list(nd);
...
static void three_lists(NODE *arg, NODE **proclst, NODE **varlst, NODE **plistlst)
{
...
static wchar_t *expand_slash(const NODE *wd)
{
...
static void po_helper_print_plist(
    void * Context,
    NODE * Key,
    NODE * Value
    )
...
static void po_helper(NODE *arg, int just_titles)  /* >0 for POT, 0 for PO, <0 for EDIT */
{
    ...
    while (proclst != NIL)
    {
        ...
        {
            err_logo(ERR_TYPES::BAD_DATA_UNREC, car(proclst));
            break;
        }
        ...
        if (tvar == UNDEFINED)
        {
            ...
            {
                err_logo(ERR_TYPES::DK_HOW_UNREC, car(proclst));
                break;
            }
        }
        else if (is_prim(tvar))
        {
            err_logo(ERR_TYPES::IS_PRIM, car(proclst));
            break;
        }
        else
        {
         ...
         current_varlist_node = cdr(current_varlist_node))
    {
        ...
        {
            err_logo(ERR_TYPES::BAD_DATA_UNREC, current_variable);
            break;
        }
        ...
        if (quoted_value == Unbound)
        {
            if (just_titles >= 0)
            {
                err_logo(ERR_TYPES::NO_VALUE, current_variable);
                break;
            }
        }
        else
    ...
    while (plistlst != NIL && NOT_THROWING)
    {
        ...
        {
            err_logo(ERR_TYPES::BAD_DATA_UNREC, car(plistlst));
            break;
        }
...
NODE *lerase(NODE *arg)
{
    ...
    while (varlst != NIL && NOT_THROWING)
    {
        ...
        {
            err_logo(ERR_TYPES::BAD_DATA_UNREC, car(varlst));
            break;
        }
...
static void bury_or_unbury_list(NODE * list, int flag, bool setflag)
{
    while (list != NIL && NOT_THROWING)
    {
        if (is_aggregate(car(list)))
        {
            err_logo(ERR_TYPES::BAD_DATA_UNREC, car(list));
            break;
        }
...
static NODE *bury_helper(NODE *arg, int flag, bool setflag)
{
    NODE *proclst;
    NODE *varlst;
    NODE *plistlst;
...
NODE *lthing(NODE *args)
{
    ...
    {
        val = err_logo(ERR_TYPES::NO_VALUE, car(args));
    }
...
static NODE *check_proctype(NODE *args, PROCTYPE wanted)
{
    ...
    NODE * procnode;
    if (wanted == PROCTYPE::PROCTYPE_Primitive)
    {
    ...
    switch (wanted)
    {
    case PROCTYPE::PROCTYPE_UserDefined:
        istype = is_prim(procnode) ? false : true;
        break;

    case PROCTYPE::PROCTYPE_Primitive:
        istype = is_prim(procnode) ? true : false;
        break;

    case PROCTYPE::PROCTYPE_Macro:
...
{
    return check_proctype(args, PROCTYPE::PROCTYPE_Primitive);
}

NODE *ldefinedp(NODE *args)
{
    return check_proctype(args, PROCTYPE::PROCTYPE_UserDefined);
}

NODE *lmacrop(NODE *args)
{
    return check_proctype(args, PROCTYPE::PROCTYPE_Macro);
}

// Copies the title line from a procedure definition
// while replacing the name of the function.
static NODE *cpdf_newname(NODE * Name, NODE * TitleLine)
{
    ...
    if (NOT_THROWING)
    {
        ...
        if (is_prim(arg))
        {
            ...
            if (min < 0)
            {
                ...
                err_logo(ERR_TYPES::SPECIAL_FORM_PROC_UNREC, car(args));
                return Unbound;
            }
            if (min == OK_NO_ARG)
            {
        ...
        else if (arg == UNDEFINED)
        {
            ...
            err_logo(ERR_TYPES::DK_HOW_UNREC, car(args));
            return Unbound;
        }
        else
        {
...
NODE *lcopydef(NODE *args)
{
    ...
    if (new_proc == UNDEFINED)
    {
        ...
        err_logo(ERR_TYPES::DK_HOW, arg2);
        return Unbound;
    }
    ...
    if (!redef && is_prim(procnode__caseobj(arg1)))
    {
        ...
        err_logo(ERR_TYPES::IS_PRIM, arg1);
        return Unbound;
    }
