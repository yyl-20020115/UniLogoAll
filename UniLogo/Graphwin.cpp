#include "PCH.h"
   ...
   #include "Parser.h"
   #include "Error.h"
   #include "UniLogo.h" // BitMapWidth, BitMapHeight
...
wxPen   g_NormalPen(*wxBLACK);                    // Handle to "Normal" Pen

wxPen   g_ErasePen(*wxWHITE);                     // Handle to "Erase" Pen
...
static FIXNUM   g_BitmapsLimit;          // number of bitmaps in g_Bitmaps array
...
{
    {{-tw, 0.0, 0.0}, { tw,  0.0, 0.0}},
    {{ tw, 0.0, 0.0}, {  0.0, tw, 0.0}},
    {{  0.0,tw, 0.0}, {-tw,  0.0, 0.0}},
    {{  tw/2.0, 0.0, 0.0}, {  tw,  tw, 0.0}},
};
...
static void GrowBitmapsArray(long NewSize)
{
    // assert that the array needs to grow
    assert(NewSize > g_BitmapsLimit);

    if ((unsigned int)NewSize > /*(signed)*/(sizeof(size_t)-1 / sizeof(*g_Bitmaps)))
    {
    ...
    for (long i = g_BitmapsLimit; i < NewSize; i++)
    {
...
{
    g_ErasePen.SetWidth(Width);
    g_ErasePen.SetColour(RedValue(Color),GreenValue(Color),BlueValue(Color));
}
...
{
    g_NormalPen.SetWidth(Width);
    g_NormalPen.SetColour(RedValue(Color),GreenValue(Color),BlueValue(Color));
}
    ...
    )
{
    ...
    {
        status = ERR_TYPES::IMAGE_GIF_SAVE_FAILED;
    }
    lsetcursorarrow(NIL);
    ...
    )
{
    ...
    if (rval != 0)
    {
        return ERR_TYPES::IMAGE_GIF_LOAD_FAILED;
    }
    ...
    )
{
    ...
    {
        TurtleScreenPosition.x = (int)g_round(TurtleWorldPosition.x);
        TurtleScreenPosition.y = (int)g_round(TurtleWorldPosition.y);
    }
...
static void InvalidateRectangleOnScreen(const RECT & ScreenRectangle)
{
    ...
    CUniLogo::GetMainFrame()->GetScreen()->RefreshRect(refreshRegion, false);
#else
        ...
        if (current_mode == mode_type::perspectivemode)
        {
            ...
            for (int j = 0; j < 4; j++)
            {
                ...
                if (ThreeD.TransformSegment(from3d, to3d, from2d, to2d))
                {
                    ...
                    g_SelectedTurtle->Points[j].from.x = (int)iFromx;
                    g_SelectedTurtle->Points[j].from.y = (int)iFromy;
                    g_SelectedTurtle->Points[j].to.x   = (int)iTox;
                    g_SelectedTurtle->Points[j].to.y   = (int)iToy;
                    g_SelectedTurtle->Points[j].bValid = (int)true;
                }
                else
                {
    ...
    if (g_SelectedTurtle->BitmapRasterMode != 0)
    {
        ...
        if (needsInvalidation)
        {
            ...
            if (g_SelectedTurtle->IsSprite)
            {
                ...
                if (current_mode == mode_type::wrapmode)
                {
...
void logofill(bool fillUntilPenColor)
{
    ...
    // events will wait until UniLogo is idle).
    screen->Update();
}
...
NODE *lclearpalette(NODE *)
{
    ...
    // UniLogo no longer supports these, but retains the
...
NODE *lbitblock(NODE *arg)
{
    ...
    // get args
    FIXNUM cutWidth  = getint(nonnegative_int_arg(arg));
    FIXNUM cutHeight = getint(nonnegative_int_arg(cdr(arg)));
    ...
    if (NOT_THROWING)
    {
        ...
        if (cutWidth != 0 && cutHeight != 0)
        {
            ...
            wxRect memoryRect(
                ...
                (int)(-turtleLocation.y + yoffset - cutHeight + LL), // top
                (int)cutWidth,                                     // width
...
// Converts the UniLogo bitmode to the Window raster mode
...
// Converts the Window raster mode to a UniLogo bitmode.
static
FIXNUM
RasterModeToBitMode(
    DWORD RasterMode
...
#ifndef WX_PURE
static bool ClipboardIsSelectedBitmap()
{
    return g_SelectedBitmap == &g_Bitmaps[0];
}
static void CopyFromBitmapArrayToClipboard()
{
...
static void PasteFromClipboardToBitmapArray()
{
    ::OpenClipboard(GetMainWindow());
...
BitCopyOrCut(NODE *arg, bool IsCut)
{
    ...
    FIXNUM tempWidth  = getint(nonnegative_int_arg(arg));
    FIXNUM tempHeight = getint(nonnegative_int_arg(cdr(arg)));
    tempWidth=tempWidth;
...
NODE *lsetturtle(NODE *args)
{
    ...
    else
    {
        ...
        if ((int)g_TurtlesLimit <= turtleId)
        {
            ...
            if ((unsigned int)(turtleId + 1) >( sizeof(size_t)-1 / sizeof(*g_Turtles)))
            {
                // this allocation would result in an integer overflow
                err_logo(ERR_TYPES::OUT_OF_MEM, NIL);
                return Unbound;
        ...
        if (turtleId > g_MaxTurtle)
        {
            ...
            for (long i = g_MaxTurtle + 1; i <= turtleId; i++)
            {
...
NODE *lsetfocus(NODE *arg)
{
    ...
    if (wc.IsSameAs(L"UniLogo",false)||
		wc.IsSameAs(L"MSWLogo Screen", false) ||
		wc.IsSameAs(L"UniLogo Screen", false))
    {
...
NODE *lwindowset(NODE *args)
{
    ...
    if (NOT_THROWING)
    {
        ...
        // For compatibility with other translations of UniLogo (with
...
NODE *lmachine(NODE *)
{
    ...
    // Get UniLogo's window dimensions
    RECT mainWindowRect;
