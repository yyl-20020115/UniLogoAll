...
mode_type current_mode = mode_type::wrapmode;
...
int GetSelectedTurtleIndex()
{
    ...
    return (int)selectedTurtleIndex;
}
    ...
    )
{
    ...
    if (!zoom_flag)
    {
        ...
        CScreen * screen = CUniLogo::GetMainFrame()->GetScreen();
        wxMemoryDC & memoryDeviceContext = screen->GetMemoryDeviceContext();
        const wxPen& op=memoryDeviceContext.GetPen();
        
        memoryDeviceContext.SetPen(pen);
        memoryDeviceContext.DrawLine(FromX, FromY, ToX, ToY);
        memoryDeviceContext.SetPen(op);
...
{
    assert(current_mode != mode_type::perspectivemode);
    ...
    if ((current_mode == mode_type::fencemode) && wrapping)
    {
        err_logo(ERR_TYPES::TURTLE_OUT_OF_BOUNDS, NIL);
    }
    else if ((current_mode == mode_type::wrapmode) && (wrapping || out_of_bounds))
    {
...
void forward_helper(FLONUM d)
{
    ...
    if (current_mode == mode_type::windowmode ||
...
NODE *lbitmapturtle(NODE * arg)
{
    ...
    if (rotatingBitmap)
    {
        ...
        if (turtleIndex < 0)
        {
            ...
            err_logo(
                ERR_TYPES::INVALID_STATE_FOR_INSTRUCTION,
...
static NODE *vec_arg_helper(NODE *Arguments, bool NegativeIsOk)
{
    ...
    while (NOT_THROWING)
    {
        ...
        setcar(Arguments, err_logo(ERR_TYPES::BAD_DATA, arg));
        arg = car(Arguments);
    }
    return Unbound;
}
...
NODE *lwrap(NODE *)
{
    ...
    current_mode = mode_type::wrapmode;
    draw_turtle(true);
    return Unbound;
}
...
{
    draw_turtle(false);
    current_mode = mode_type::fencemode;
    draw_turtle(true);
    return Unbound;
}
...
{
    draw_turtle(false);
    current_mode = mode_type::windowmode;
    draw_turtle(true);
...
{
    draw_turtle(false);
    current_mode = mode_type::perspectivemode;
...
{
#ifndef WX_PURE
    CStringPrintedNode textbuf(car(arg), CStringPrintedNode::PRINTLIMITSPECIFIER::WithPrintLimits);
...
NODE *lsetpensize(NODE *args)
{
    ...
    set_pen_width((int)width);
    set_pen_height((int)height);
...
NODE *lsetpenpattern(NODE *args)
{
    ...
    {
        assign(arg, err_logo(ERR_TYPES::BAD_DATA, arg));
    }
