...
void CClientRectangle::InitializeFromInput(NODE * & Args)
{
    ...
    SetX((int)int_arg(nextinput));
    nextinput = cdr(nextinput);

    SetY((int)int_arg(nextinput));
    nextinput = cdr(nextinput);

    SetWidth((int)getint(nonnegative_int_arg(nextinput)));
    nextinput = cdr(nextinput);

    SetHeight((int)getint(nonnegative_int_arg(nextinput)));
    nextinput = cdr(nextinput);
...
public:
            ...
            wxCLIP_CHILDREN | wxCAPTION)
    {
        ...
        // Since the UniLogo API does not have this behavior, we must disable it.
...
public:
        ...
        )
    {
        ...
        // MSWLogo used this style, which means that UniLogo must also, or
        ...
        // correctly when the COMBOBOX is put on the UniLogo screen window and
...
wxWindow * CLogoWidget::GetWindow() const
{
    switch (m_Type)
    {
        case WINDOWTYPE::WINDOWTYPE_Window:
    case WINDOWTYPE::WINDOWTYPE_Dialog:
        return Dialog;

    case WINDOWTYPE::WINDOWTYPE_Static:
        return StaticText;

    case WINDOWTYPE::WINDOWTYPE_ListBox:
        return ListBox;

    case WINDOWTYPE::WINDOWTYPE_ComboBox:
        return ComboBox;

    case WINDOWTYPE::WINDOWTYPE_Button:
        return Button;

    case WINDOWTYPE::WINDOWTYPE_ScrollBar:
        return ScrollBar;

    case WINDOWTYPE::WINDOWTYPE_GroupBox:
        return GroupBox;

    case WINDOWTYPE::WINDOWTYPE_RadioButton:
        return RadioButton;

    case WINDOWTYPE::WINDOWTYPE_CheckBox:
        return CheckBox;

    case WINDOWTYPE::WINDOWTYPE_None:
        assert(!"can't happen");
        return NULL;
    }
    ...
    if (p != NULL)
    {
        ...
        if (level == 0)
        {
            const wxString & temp = wxString::Format(
                ...
                p->m_Key);
            PutIntoComboBox((temp), MESSAGETYPE::Normal);
        }
        ...
        do
        {
                ...
                ff->m_Parent.IsSameAs(k))
            {
                ...
                PutIntoComboBox(/*WXSTRING_TO_STRING*/(temp), MESSAGETYPE::Normal);
...
NODE *lwindowcreate(NODE *args)
{
    ...
    // get all the args
	wxString parentname = ArgumentUtils::cnv_strnode_string(nextArg);
    nextArg = cdr(nextArg);

    wxString childname = ArgumentUtils::cnv_strnode_string(nextArg);
    nextArg = cdr(nextArg);
    ...
    if (g_LogoWidgets.get(childname) != NULL)
    {
        ...
        err_logo(ERR_TYPES::WINDOW_ALREADY_EXISTS, cadr(args));
        return Unbound;
    }
    ...
    CLogoWidget *child = new CLogoWidget(WINDOWTYPE::WINDOWTYPE_Window, childname);

    // if parent exists use it, else use desktop
    CLogoWidget *parent = g_LogoWidgets.get(parentname, WINDOWTYPE::WINDOWTYPE_Window);
    wxWindow * wxParent;
    if (parent != NULL)
    {
...
{
    wxString childname = ArgumentUtils::cnv_strnode_string(args);
    ...
    if (window == NULL)
    {
        err_logo(ERR_TYPES::WINDOW_DOES_NOT_EXIST, car(args));
        return Unbound;
    }
...
{
    return WindowEnableHelper(args, WINDOWTYPE::WINDOWTYPE_Window);
}

NODE *lwindowdelete(NODE *arg)
{
    wxString windowname = ArgumentUtils::cnv_strnode_string(arg);

    if (NOT_THROWING)
    {
        CLogoWidget *window = g_LogoWidgets.get(windowname, WINDOWTYPE::WINDOWTYPE_Window);
        if (window != NULL)
        {
...
NODE *ldialogcreate(NODE *args)
{
    ...
    // make one
    CLogoWidget * child = new CLogoWidget(WINDOWTYPE::WINDOWTYPE_Dialog, childname);
    ...
    child = g_LogoWidgets.get(childname, WINDOWTYPE::WINDOWTYPE_Dialog);
    if (child != NULL)
    {
...
{
    return WindowEnableHelper(args, WINDOWTYPE::WINDOWTYPE_Dialog);
}
   
NODE *ldialogdelete(NODE *args)
{
    return WindowDeleteHelper(args, WINDOWTYPE::WINDOWTYPE_Dialog);
}
...
NODE *llistboxcreate(NODE *args)
{
    ...
    CLogoWidget * child = new CLogoWidget(WINDOWTYPE::WINDOWTYPE_ListBox, childname);

    CLogoWidget *parent = g_LogoWidgets.get(parentname, WINDOWTYPE::WINDOWTYPE_Window, WINDOWTYPE::WINDOWTYPE_Dialog);
    if (parent != NULL)
    {
...
{
    return WindowEnableHelper(args, WINDOWTYPE::WINDOWTYPE_ListBox);
}

NODE *llistboxdelete(NODE *args)
{
    return WindowDeleteHelper(args, WINDOWTYPE::WINDOWTYPE_ListBox);
}
...
NODE *llistboxgetselect(NODE *args)
{
    ...
    CLogoWidget * listbox = g_LogoWidgets.get(listboxname, WINDOWTYPE::WINDOWTYPE_ListBox);
    if (listbox == NULL)
    {
...
NODE *llistboxdeletestring(NODE *args)
{
    ...
    if (listbox->ListBox->GetCount() <= (unsigned int) index)
    {
        ...
        // This is not an error in UniLogo, but it is in wxWidgets,
...
NODE *lcomboboxcreate(NODE *args)
{
    ...
    CLogoWidget * child = new CLogoWidget(WINDOWTYPE::WINDOWTYPE_ComboBox, childname);
...
{
    return WindowEnableHelper(args, WINDOWTYPE::WINDOWTYPE_ComboBox);
}

NODE *lcomboboxdelete(NODE *args)
{
    return WindowDeleteHelper(args, WINDOWTYPE::WINDOWTYPE_ComboBox);
}

NODE *lcomboboxgettext(NODE *args)
{
    // get args
	wxString comboboxname = ArgumentUtils::cnv_strnode_string(args);
    // get the combobox
    CLogoWidget *combobox = g_LogoWidgets.get(comboboxname, WINDOWTYPE::WINDOWTYPE_ComboBox);
    if (combobox == NULL)
    {
...
NODE *lscrollbarcreate(NODE *args)
{
    ...
    CLogoWidget * child = new CLogoWidget(WINDOWTYPE::WINDOWTYPE_ScrollBar, childname);
...
NODE *lscrollbarset(NODE *args)
{
    ...
    FIXNUM lo = int_arg(nextArg);
    nextArg = cdr(nextArg);

    FIXNUM hi = int_arg(nextArg);
    if (hi < lo)
    {
        // the low value must be less than the high value
        NODE * node = err_logo(ERR_TYPES::BAD_DATA, car(nextArg));
        gcref(node);
    }
    nextArg = cdr(nextArg);
    ...
    CLogoWidget *scrollbar = g_LogoWidgets.get(scrollbarname, WINDOWTYPE::WINDOWTYPE_ScrollBar);
    if (scrollbar == NULL)
    {
        err_logo(ERR_TYPES::WINDOW_DOES_NOT_EXIST, car(args));
        return Unbound;
    ...
    scrollbar->ScrollBar->Set((int)lo, (int)hi, pos);
    return Unbound;
}
...
{
    return WindowEnableHelper(args, WINDOWTYPE::WINDOWTYPE_ScrollBar);
}

NODE *lscrollbardelete(NODE *args)
{
    return WindowDeleteHelper(args, WINDOWTYPE::WINDOWTYPE_ScrollBar);
}
...
NODE *lstaticcreate(NODE *args)
{
    ...
    CLogoWidget * child = new CLogoWidget(WINDOWTYPE::WINDOWTYPE_Static, childname);

    CLogoWidget *parent = g_LogoWidgets.get(
        parentname,
        WINDOWTYPE::WINDOWTYPE_Window,
...
NODE *lstaticupdate(NODE *args)
{
    ...
    CLogoWidget *temp = g_LogoWidgets.get(childname, WINDOWTYPE::WINDOWTYPE_Static);
    if (temp == NULL)
    {
        err_logo(ERR_TYPES::WINDOW_DOES_NOT_EXIST, car(args));
        return Unbound;
...
{
    return WindowDeleteHelper(args, WINDOWTYPE::WINDOWTYPE_Static);
}
...
NODE *lbuttoncreate(NODE *args)
{
    ...
    CLogoWidget * child = new CLogoWidget(WINDOWTYPE::WINDOWTYPE_Button, childname);
...
{
    wxString buttonname = ArgumentUtils::cnv_strnode_string(args);
    ...
    CLogoWidget *button = g_LogoWidgets.get(buttonname, WINDOWTYPE::WINDOWTYPE_Button);
    if (button == NULL)
    {
        err_logo(ERR_TYPES::WINDOW_DOES_NOT_EXIST, car(args));
        return Unbound;
...
{
    return WindowEnableHelper(args, WINDOWTYPE::WINDOWTYPE_Button);
}

NODE *lbuttondelete(NODE *args)
{
    return WindowDeleteHelper(args, WINDOWTYPE::WINDOWTYPE_Button);
}
...
NODE *lgroupboxcreate(NODE *args)
{
    ...
    CLogoWidget * child = new CLogoWidget(WINDOWTYPE::WINDOWTYPE_GroupBox, childname);
...
{
    return WindowDeleteHelper(args, WINDOWTYPE::WINDOWTYPE_GroupBox);
}
...
NODE *lradiobuttoncreate(NODE *args)
{
	...
	wxString groupname = ArgumentUtils::cnv_strnode_string(nextArg);
    nextArg = cdr(nextArg);
    ...
    CLogoWidget *group = g_LogoWidgets.get(groupname, WINDOWTYPE::WINDOWTYPE_GroupBox);
    if (group == NULL)
    {
        // the group does not exist
        err_logo(ERR_TYPES::WINDOW_DOES_NOT_EXIST, cadr(args));
        return Unbound;
    }
    ...
    if (g_LogoWidgets.get(childname) != NULL)
    {
        ...
        err_logo(ERR_TYPES::WINDOW_ALREADY_EXISTS, car(cdr(cdr(args))));
        return Unbound;
    }
     
    CLogoWidget * child = new CLogoWidget(WINDOWTYPE::WINDOWTYPE_RadioButton, childname);
...
{
    return WindowEnableHelper(args, WINDOWTYPE::WINDOWTYPE_RadioButton);
}

NODE *lradiobuttondelete(NODE *args)
{
    return WindowDeleteHelper(args, WINDOWTYPE::WINDOWTYPE_RadioButton);
}
...
NODE *lradiobuttonget(NODE *args)
{
    ...
    CLogoWidget *radiobutton = g_LogoWidgets.get(radiobuttonname,WINDOWTYPE::WINDOWTYPE_RadioButton);
    if (radiobutton == NULL)
    {
...
NODE *lcheckboxcreate(NODE *args)
{
    ...
    CLogoWidget * child = new CLogoWidget(WINDOWTYPE::WINDOWTYPE_CheckBox, childname);
...
{
    return WindowEnableHelper(args, WINDOWTYPE::WINDOWTYPE_CheckBox);
}

NODE *lcheckboxdelete(NODE *args)
{
    return WindowDeleteHelper(args, WINDOWTYPE::WINDOWTYPE_CheckBox);
}
...
NODE *lcheckboxget(NODE *args)
{
    ...
    CLogoWidget *checkbox = g_LogoWidgets.get(checkboxname, WINDOWTYPE::WINDOWTYPE_CheckBox);
    if (checkbox == NULL)
    {
...
NODE *ldebugwindows(NODE *arg)
{
    if (arg != NIL)
    {
        ...
        {
            err_logo(ERR_TYPES::WINDOW_DOES_NOT_EXIST, car(arg));
            return Unbound;
        }
...
{
	wxFileName filename = ArgumentUtils::cnv_strnode_string(args);
...
{
	wxString filename = ArgumentUtils::cnv_strnode_string(args);
	edit_editexit = ArgumentUtils::cnv_strnode_string(cdr(args));
