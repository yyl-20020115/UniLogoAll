...
INPUTMODE input_mode = INPUTMODE::INPUTMODE_None;
...
{
	if (input_mode == INPUTMODE::INPUTMODE_To && strm->GetFile() == stdin)
	{
		g_ReadBuffer.Empty();
	}
}
wchar_t rd_fgetwc(CTextStream *stream)
{
	...
	else //strm == stdin (actually it's the buffer)
	{
		...
		if (g_ReadBuffer.IsEmpty())
		{
			...
			case INPUTMODE::INPUTMODE_Pause:
...
// Prompt     - Mostly ignored by UniLogo.  If it's "RW", then
//              the characters "(", ")", "{", "}", "[", "]", and ';'
...
NODE *reader(CTextStream *fileStream, const wchar_t * Prompt)
{
	...
	if (!setjmp(iblk_buf))
	{
		...
		while (c != (signed)WEOF && (c != L'\n' || vbar || paren || bracket || brace))
		{
			...
			if (input_mode == INPUTMODE::INPUTMODE_To)
			{
				...
				if (uncapital(c) == uncapital(*enderProgress))
				{
					...
					if (*enderProgress == L'\0')
					{
						...
						// inside a [, |, (, or {.
						err_logo(ERR_TYPES::DEEPEND, deepend_proc_name);
						break;
					}
				}
				else
...
)
{
	...
	do
	{
		...
		{
			err_logo(ERR_TYPES::UNEXPECTED_BRACKET, NIL);
		}
		else if (ch == L'}')
		{
			err_logo(ERR_TYPES::UNEXPECTED_BRACE, NIL);
		}
		else if (ch == L'[')
		{
			...
			if (NOT_THROWING)
			{
				...
				if (**inln == L'@')
				{
					...
					*inln = unparsedPortion;
					...
					setarrorg(car(tnode),(int) origin);
				}
				if (**inln == L'\0')
				{
					ch = L'\0';
			...
			**inln == L'{' || **inln == L'}')
		{
			...
			if (word_length > 0)
			{
				...
				wchar_t * (*copyRoutine) (wchar_t *, const wchar_t *, size_t);
...
NODE *runparse(NODE *ndlist)
{
	...
	if (!is_list(ndlist))
	{
		err_logo(ERR_TYPES::BAD_DATA_UNREC, ndlist);
		return NIL;
	}
