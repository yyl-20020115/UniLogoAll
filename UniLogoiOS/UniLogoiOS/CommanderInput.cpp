    ...
    CLogoCodeCtrl(Parent, Id)
{
    ...
    acceleratorEntries[7].Set(wxACCEL_CTRL, ']', (int)CKID::ID_FINDMATCHINGPAREN);

    // Ctrl+Shift+] selects to matching paren
    acceleratorEntries[8].Set(
        wxACCEL_CTRL | wxACCEL_SHIFT,
        ']',
        (int)CKID::ID_SELECTMATCHINGPAREN);

    // Ctrl+Space does auto-complete
    acceleratorEntries[9].Set(wxACCEL_CTRL, WXK_SPACE, (int)CKID::ID_AUTOCOMPLETE);
...
void CCommanderInput::OnClipboardPaste(wxStyledTextEvent& Event)
{
    ...
    int lastIndex;
    for (lastIndex = (int)pastedText.length() - 1; 0 <= lastIndex; lastIndex--)
    {
        if (pastedText[lastIndex] == L'\\')
        {
            // We shouldn't remove a quoted newline,
...
BEGIN_EVENT_TABLE(CCommanderInput, CLogoCodeCtrl)
    ...
    EVT_MENU((int)CKID::ID_FINDMATCHINGPAREN,   CCommanderInput::OnFindMatchingParen)
    EVT_MENU((int)CKID::ID_SELECTMATCHINGPAREN, CCommanderInput::OnSelectMatchingParen)
    EVT_MENU((int)CKID::ID_AUTOCOMPLETE,        CCommanderInput::OnAutoComplete)
#ifndef USE_RICHTEXT_CODE_EDITOR
