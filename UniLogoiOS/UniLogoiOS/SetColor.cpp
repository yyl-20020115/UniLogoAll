      ...
      m_LogoCommand(LogoCommand)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        wxWindow * picture = new CColorWindow(
            this,
            (int)ID_SETCOLOR::ID_SETCOLOR_COLOR1 + i,
            wxDefaultPosition,
            wxSize(width, height),
    ...
    //
    for (int i = 0; i < ARRAYSIZE(sliderData); i++)
    {
        ...
        *sliderData[i].Slider = new wxSlider(
            this,
            (int)ID_SETCOLOR::ID_SETCOLOR_SLIDER_RED + i,
    ...
    wxButton *apply = new wxButton(
        this, 
        (int)ID_SETCOLOR::ID_SETCOLOR_APPLY,
...
void CSetColor::OnApplyButton(wxCommandEvent& event)
{
    ...
    RunLogoInstructionFromGui(wxString::Format(L"%s [%d %d %d]", ArgumentUtils::NormalizeCaseForDisplay(m_LogoCommand),red,green,blue));
}
...
BEGIN_EVENT_TABLE(CSetColor, wxDialog)
    ...
    EVT_BUTTON((int)ID_SETCOLOR::ID_SETCOLOR_APPLY,        CSetColor::OnApplyButton)
    EVT_SLIDER((int)ID_SETCOLOR::ID_SETCOLOR_SLIDER_RED,   CSetColor::OnSliderUpdated)
    EVT_SLIDER((int)ID_SETCOLOR::ID_SETCOLOR_SLIDER_GREEN, CSetColor::OnSliderUpdated)
    EVT_SLIDER((int)ID_SETCOLOR::ID_SETCOLOR_SLIDER_BLUE,  CSetColor::OnSliderUpdated)
END_EVENT_TABLE()
