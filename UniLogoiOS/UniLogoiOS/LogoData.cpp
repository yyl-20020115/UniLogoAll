...
wchar_t *strnzcpy(wchar_t *dst, const wchar_t *src, size_t len)
{
    wcsncpy(dst, src, len);
    dst[len] = L'\0';
    return dst;
...
wchar_t *word_strnzcpy(wchar_t *dst, NODE *wordlist, size_t len)
{
	wchar_t *temp = dst;
...
wchar_t *noparity_strnzcpy(wchar_t *dst, const wchar_t * src, size_t len)
{
    for (int i = 0; i < len; i++)
    {
...
wchar_t *mend_strnzcpy(wchar_t *dst, const wchar_t * src, size_t len)
{
    bool vbar = false;
...
wchar_t *mend_nosemi(wchar_t *dst, const wchar_t * src, size_t len)
{
    bool vbar = false;
...
wchar_t *quote_strnzcpy(wchar_t *dst, const wchar_t *src, size_t len)
{
    dst[0] = L'"';
...
wchar_t *colon_strnzcpy(wchar_t *dst, const wchar_t * src, size_t len)
{
    dst[0] = L':';
    wcsncpy(dst + 1, src, len - 1);
    dst[len] = L'\0';
...
wchar_t *low_strnzcpy(wchar_t *dst, const wchar_t * src, size_t len)
{
#ifdef WX_PURE
    for (int i = 0; i < len; i++)
    {
...
wchar_t *cap_strnzcpy(wchar_t *dst, const wchar_t * src, size_t len)
{
#ifdef WX_PURE
    for (int i = 0; i < len; i++)
    {
...
wchar_t * NormalizeCaseForDisplay(wchar_t *dst, const wchar_t * src, size_t len)
{
    return cap_strnzcpy(dst, src, len);
}

wchar_t *noparitylow_strnzcpy(wchar_t *dst, const wchar_t *src, size_t len)
{
	wchar_t *temp = dst;
...
int low_strncmp(const wchar_t *string1, const wchar_t * string2, size_t length)
{
#ifdef WX_PURE
...
int noparity_strncmp(const wchar_t * s1, const wchar_t * s2, size_t len)
{
    for (int i = 0; i < len; i++)
    {
...
int noparitylow_strncmp(const wchar_t * s1, const wchar_t * s2, size_t len)
{
    for (int i = 0; i < len; i++)
    {
...
make_strnode(
    ...
    int         len,
    NODETYPES   typ,  
	wchar_t *    (*copy_routine) (wchar_t *, const wchar_t *, size_t)
    )
{
...
// 0,  if NodeA == NodeB
// >0, if NodeA > NodeB
long ComparePropertyListKeys(NODE * NodeA, NODE * NodeB)
{
                    ...
                    if (is_list(NodeA))
                    {
                        if (is_list(NodeB))
                        {
                            ...
                            while (cursorA != NIL && cursorB != NIL)
                            {
                                ...
                                long rval = ComparePropertyListKeys(elementA, elementB);
                                if (rval != A_EQUALS_B)
                                {
...
NODE * llogoversion(NODE * args)
{
    ...
    NODE * application_version = make_static_strnode(UNILOGO_VERSION);
    NODE * os_name             =
#ifdef __APPLE__
    make_static_strnode(L"MacOS")
#else
#ifdef __LINUX__
    make_static_strnode(L"Linux")
#else
    make_static_strnode(L"Windows")
#endif
#endif
    ;
