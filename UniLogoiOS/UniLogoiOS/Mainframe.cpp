#include "PCH.h"
    ...
    #include "FilesOp.h"
    #include "Graphwin.h"
    #include "MM.h" // for uninitialize_timers()
    #include "Startup.h"
    #include "FontUtils.h"
    #include "Net.h"
...
enum class MainFrameMenuIds :int
{
...

{
    ...
    static const MENUITEM fileMenuItems[] = {
        {(GetResourceString(L"LOCALIZED_FILE_NEW")),              (int)MainFrameMenuIds::ID_FILENEW},
...
#endif
        {L"",0},
        {GetResourceString(L"LOCALIZED_FILE_EDIT"),             (int)MainFrameMenuIds::ID_FILEEDIT},
        {GetResourceString(L"LOCALIZED_FILE_ERASE"),            (int)MainFrameMenuIds::ID_FILEERASE},
        {L"",0},
        {GetResourceString(L"LOCALIZED_FILE_EXIT"),             wxID_EXIT},
    };

    static const MENUITEM bitmapMenuItems[] = {
        {GetResourceString(L"LOCALIZED_BITMAP_NEW"),           (int)MainFrameMenuIds::ID_BITMAPNEW},
        {GetResourceString(L"LOCALIZED_BITMAP_LOAD"),          (int)MainFrameMenuIds::ID_BITMAPOPEN},
        {GetResourceString(L"LOCALIZED_BITMAP_SAVE"),          (int)MainFrameMenuIds::ID_BITMAPSAVE},
        {GetResourceString(L"LOCALIZED_BITMAP_SAVEAS"),        (int)MainFrameMenuIds::ID_BITMAPSAVEAS},
//        {L"",0},
...
//        {GetResourceString(L"LOCALIZED_BITMAP_PRINTERSETUP"),  ID_BITMAPPRINTERSETUP},
        {L"",0},
        {GetResourceString(L"LOCALIZED_BITMAP_ACTIVEAREA"),    (int)MainFrameMenuIds::ID_BITMAPPRINTERAREA},
    };

    static const MENUITEM setMenuItems[] = {
        {GetResourceString(L"LOCALIZED_SET_PENSIZE"),       (int)MainFrameMenuIds::ID_SETPENSIZE},
        {L"",0},
        {GetResourceString(L"LOCALIZED_SET_LABELFONT"),     (int)MainFrameMenuIds::ID_SETLABELFONT},
        {GetResourceString(L"LOCALIZED_SET_COMMANDERFONT"), (int)MainFrameMenuIds::ID_SETCOMMANDERFONT},
        {L"",},
        {GetResourceString(L"LOCALIZED_SET_PENCOLOR"),      (int)MainFrameMenuIds::ID_SETPENCOLOR},
        {GetResourceString(L"LOCALIZED_SET_FLOODCOLOR"),    (int)MainFrameMenuIds::ID_SETFLOODCOLOR},
        {GetResourceString(L"LOCALIZED_SET_SCREENCOLOR"),   (int)MainFrameMenuIds::ID_SETSCREENCOLOR},
    };
...
#if MANUAL_HAS_TRANSLATION_TABLES
        // options for translating to/from English
//        {GetResourceString(L"LOCALIZED_HELP_LANGTOENGLISH"), (int)MainFrameMenuIds::ID_HELPLANGTOENGLISH},
//        {GetResourceString(L"LOCALIZED_HELP_ENGLISHTOLANG"), (int)MainFrameMenuIds::ID_HELPENGLISHTOLANG},
#endif
//        {L"",0},
//        {GetResourceString(L"LOCALIZED_HELP_TUTORIAL"),      (int)MainFrameMenuIds::ID_HELPTUTORIAL},
//        {GetResourceString(L"LOCALIZED_HELP_DEMO"),          (int)MainFrameMenuIds::ID_HELPDEMO},
//        {GetResourceString(L"LOCALIZED_HELP_EXAMPLE"),       (int)MainFrameMenuIds::ID_HELPEXAMPLES},
//        {GetResourceString(L"LOCALIZED_HELP_RELEASENOTES"),  (int)MainFrameMenuIds::ID_HELPRELEASENOTES},
//        {L"",0},
        {GetResourceString(L"LOCALIZED_HELP_ABOUTUNILOGO"),  wxID_ABOUT},
        //{GetResourceString(L"LOCALIZED_HELP_MS"),            (int)MainFrameMenuIds::ID_HELPABOUTMS},
    };
#ifndef __WXOSX_IPHONE__
    // Make a menubar
    ...
    m_Splitter = new wxSplitterWindow(
        ...
        wxDefaultPosition,
        wxDefaultSize,
        wxSP_LIVE_UPDATE | wxCLIP_CHILDREN);//wxSP_3D | wxSP_LIVE_UPDATE | wxCLIP_CHILDREN | wxSP_NO_XP_THEME);
...
void CMainFrame::DockCommanderWindow()
{
    if (!m_CommanderIsDocked) 
    {
        ...
        if (m_Commander != NULL)
        {
            ...
            // idle.  This causes problems for test UniLogo test pass, as
            // UniLogo may not become idle if running many commands, leading to
    ...
    if (DirectionFlags & wxNavigationKeyEvent::IsForward)
    {
        if (CurrentWindowFocus == m_Screen)
        {
            if (m_CommanderIsDocked)
            {
                ...
                // This is the current UniLogo behavior and there's
...
void CMainFrame::OnResize(wxSizeEvent& Event)
{
    ...
    {
        // If UniLogo was started with the -F, then it is supposed
        ...
        // Therefore, when UniLogo is maximized and there are no expectations
...
void CMainFrame::OnClose(wxCloseEvent& Event)
{
    ...
    // or if UniLogo was started with the -F option.
...
{
    // For compatability with MSWLogo, when UniLogo is able to process
...
static void EraseContentsOfWorkspace()
{
    ...
    // workspace was to restore UniLogo to its original state (not to erase
    ...
    //     COPYDEF "MYPROC "PROC
    //
    // Then UniLogo would attempt to load "PROC.LGO from the current working
    ...
    //
    // To avoid this, or rather to have the same behavior as when UniLogo first
    // started, we switch to the UniLogo directory before loading the
    // startup.logoscript file.
    const wxString savedWorkingDirectory(wxGetCwd());
    wxSetWorkingDirectory(g_UnilogoBaseDirectory);

    // Load the startup script.
    fileload(g_UnilogoBaseDirectory + L"startup.logoscript",true,0);
...
// Gets the full path to where the UniLogo screensaver
// should be located (if it's installed).
static
wxString
GetScreenSaverFilePath()
{
    static const wchar_t screenSaverFileName[] = L"\\unilogo.scr";
...
void CMainFrame::OnFileSetAsScreenSaver(wxCommandEvent& WXUNUSED(Event))
{
    ...
    if (startup == Unbound)
    {
        explainText = CSelectStartupInstructionDialog::EXPLAINTEXT::StartupNotDefined;
    }
    else if (startup == NIL)
    {
        explainText = CSelectStartupInstructionDialog::EXPLAINTEXT::StartupEmpty;
    }
    else if (!is_list(startup))
    {
        explainText = CSelectStartupInstructionDialog::EXPLAINTEXT::StartupNotList;
    }
    else
    {
        explainText = CSelectStartupInstructionDialog::EXPLAINTEXT::None;
    }

    if (explainText != CSelectStartupInstructionDialog::EXPLAINTEXT::None)
    {
...
void CMainFrame::OnBitmapPrint(wxCommandEvent& WXUNUSED(Event))
{
    ...
    // UniLogo always scales the active area of the screen to fit on a
...
{
    OpenFileWithDefaultApplication(g_UnilogoBaseDirectory + L"EXAMPLES\\INDEX.HTML");
}

void CMainFrame::OnHelpReleaseNotes(wxCommandEvent& WXUNUSED(Event))
{
    OpenFileWithDefaultApplication(g_UnilogoBaseDirectory + L"README.TXT");
}

void CMainFrame::OnAboutUniLogo(wxCommandEvent& WXUNUSED(Event))
{
#ifndef __WXOSX_IPHONE__
    // show the "About UniLogo" dialog box
    CAboutUniLogo dlg(this);
    auto r = this->GetRect();
    auto m = dlg.GetRect();
    dlg.SetPosition(wxPoint(r.x + (r.width - m.width)/2,r.y + (r.height - m.height)/2));
    dlg.ShowModal();
#endif
}

void CMainFrame::PostCheckQueueMessage()
{
//#ifndef WX_PURE
//    PostMessage(
//        static_cast<HWND>(GetHandle()),
...
EVT_MENU((int)MainFrameMenuIds::ID_FILENEW, CMainFrame::OnFileNew)
EVT_MENU((int)MainFrameMenuIds::ID_FILELOAD, CMainFrame::OnFileLoad)
EVT_MENU((int)MainFrameMenuIds::ID_FILEOPEN, CMainFrame::OnFileOpen)
EVT_MENU((int)MainFrameMenuIds::ID_FILESAVE, CMainFrame::OnFileSave)
EVT_MENU((int)MainFrameMenuIds::ID_FILESAVEAS, CMainFrame::OnFileSaveAs)
EVT_MENU((int)MainFrameMenuIds::ID_FILESAVEAS_UTF8, CMainFrame::OnFileSaveAsUTF8)
EVT_MENU((int)MainFrameMenuIds::ID_FILESAVEAS_UTF16, CMainFrame::OnFileSaveAsUTF16)
EVT_MENU((int)MainFrameMenuIds::ID_FILESETASSCREENSAVER, CMainFrame::OnFileSetAsScreenSaver)
EVT_UPDATE_UI((int)MainFrameMenuIds::ID_FILESETASSCREENSAVER, CMainFrame::OnUpdateFileSetAsScreenSaver)
EVT_MENU((int)MainFrameMenuIds::ID_FILEEDIT, CMainFrame::OnEditProcedure)
EVT_MENU((int)MainFrameMenuIds::ID_FILEERASE, CMainFrame::OnEraseProcedure)
EVT_MENU(wxID_EXIT, CMainFrame::OnExit)
EVT_MENU((int)MainFrameMenuIds::ID_BITMAPNEW, CMainFrame::OnBitmapNew)
EVT_MENU((int)MainFrameMenuIds::ID_BITMAPOPEN, CMainFrame::OnBitmapOpen)
EVT_MENU((int)MainFrameMenuIds::ID_BITMAPSAVE, CMainFrame::OnBitmapSave)
EVT_MENU((int)MainFrameMenuIds::ID_BITMAPSAVEAS, CMainFrame::OnBitmapSaveAs)
EVT_MENU((int)MainFrameMenuIds::ID_BITMAPPRINT, CMainFrame::OnBitmapPrint)
EVT_MENU((int)MainFrameMenuIds::ID_BITMAPPRINTERSETUP, CMainFrame::OnBitmapPrinterSetup)
EVT_MENU((int)MainFrameMenuIds::ID_BITMAPPRINTERAREA, CMainFrame::OnSetActiveArea)
EVT_MENU((int)MainFrameMenuIds::ID_SETPENSIZE, CMainFrame::OnSetPenSize)
EVT_MENU((int)MainFrameMenuIds::ID_SETLABELFONT, CMainFrame::OnSetLabelFont)
EVT_MENU((int)MainFrameMenuIds::ID_SETCOMMANDERFONT, CMainFrame::OnSetCommanderFont)
EVT_MENU((int)MainFrameMenuIds::ID_SETPENCOLOR, CMainFrame::OnSetPenColor)
EVT_MENU((int)MainFrameMenuIds::ID_SETSCREENCOLOR, CMainFrame::OnSetScreenColor)
EVT_MENU((int)MainFrameMenuIds::ID_SETFLOODCOLOR, CMainFrame::OnSetFloodColor)
EVT_MENU(wxID_HELP_INDEX, CMainFrame::OnHelp)
#if MANUAL_HAS_TRANSLATION_TABLES
// options for translating to/from English
EVT_MENU((int)MainFrameMenuIds::ID_HELPLANGTOENGLISH, CMainFrame::OnHelpLanguageToEnglish)
EVT_MENU((int)MainFrameMenuIds::ID_HELPENGLISHTOLANG, CMainFrame::OnHelpEnglishToLanguage)
#endif
EVT_MENU((int)MainFrameMenuIds::ID_HELPTUTORIAL, CMainFrame::OnHelpTutorial)
EVT_MENU((int)MainFrameMenuIds::ID_HELPDEMO, CMainFrame::OnHelpDemo)
EVT_MENU((int)MainFrameMenuIds::ID_HELPEXAMPLES, CMainFrame::OnHelpExamples)
EVT_MENU((int)MainFrameMenuIds::ID_HELPRELEASENOTES, CMainFrame::OnHelpReleaseNotes)
EVT_MENU(wxID_ABOUT, CMainFrame::OnAboutUniLogo)
