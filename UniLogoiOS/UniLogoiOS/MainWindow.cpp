...
static ERR_TYPES WriteDIB(FILE* File, int MaxBitmapBitDepth)
{
    ERR_TYPES status = ERR_TYPES::SUCCESS;

#ifndef _WINDOWS
    status = ERR_TYPES::FILE_ERROR;
#else
    // grab a DC
...
ERR_TYPES DumpBitmapFile(const wxString& Filename, int MaxBitCount)
{
    ...
    if (file == NULL)
    {
        return ERR_TYPES::IMAGE_BMP_OPEN_FAILED;
    }
    ...
    if (rval != 0)
    {
        status = ERR_TYPES::IMAGE_BMP_WRITE_FAILED;
    }
    ...
    )
{
    ...
    {
        // not a bitmap
        status = ERR_TYPES::IMAGE_BMP_INVALID;
    }
    fclose(file);
    ...
    )
{
    ...
    if (!zoom_flag)
    {
             ...
             regionIterator++)
        {
            ...
            wxCoord x      = (int)regionIterator.GetX();
            wxCoord y      = (int)regionIterator.GetY();
            wxCoord width  = (int)regionIterator.GetWidth();
            wxCoord height = (int)regionIterator.GetHeight();
