...
#include "WorkSpace.h" // for g_CharactersSuccessfullyParsedInEditor
#endif
...
private:
	...
	// by the printer.  UniLogo always uses the physical margins.
	//
	// m_RenderRectangle is the region (relative to the physical
	// margins of the printer) where UniLogo prints its content.
	//
...
void CLogoCodeCtrl::Print()
{
	...
	// UniLogo's editor doesn't support advanced printer options,
	...
	m_IsDirty(false)
{
	...
	// Switch to the UniLogo style
	StyleClearAll();
	if (TheLanguageModule == 0)
	{
		Catalogue::AddLexerModule(TheLanguageModule = &lmUniLogo);
	}
	SetLexer(SCLEX_UNILOGO);
	Colourise(0, -1);
	...
	AutoCompSetIgnoreCase(true); // UniLogo is case insensitive
...
CLogoCodeCtrl::SetFont(const wxFont & Font)
{
	...
	StyleSetForeground(SCE_UNI_COMMENT, darkgreen);
	StyleSetForeground(SCE_UNI_COMMENTBACKSLASH, darkgreen);
	StyleSetForeground(SCE_UNI_STRING, darkred);
	StyleSetForeground(SCE_UNI_STRING_VBAR, darkred);

	StyleSetForeground(SCE_UNI_VARIABLE, darkblue);
	StyleSetForeground(SCE_UNI_VARIABLE_VBAR, darkblue);
...
void CLogoCodeCtrl::AutoComplete()
{
	...
	if (caretStyle == SCE_UNI_DEFAULT || caretStyle == SCE_UNI_IDENTIFIER)
	{
	...
	else if (caretStyle == SCE_UNI_VARIABLE)
	{
	...
	// Because UniLogo is case-insensitive, a programmer can use whatever
	...
	if ((completionPrefix.Upper()) == completionPrefix)
	{
		...
		casePreference = CasePreference::ALLCAPS;
	}
	else if ((completionPrefix.Lower()) == completionPrefix)
	{
		casePreference = CasePreference::LOWERCASE;
	}
	else
	{
		// Assume any mixed case style is initial caps.
		casePreference = CasePreference::INITIALCAPS;
	}
	...
	for (NODE * nd = allNames; nd != NIL; nd = cdr(nd))
	{
		...
		// Because UniLogo is case-insensitive, the auto-completion
		...
		if (casePreference == CasePreference::ALLCAPS)
		{
	...
	{
		AutoCompShow((int)completionPrefixLength, completions);
	}
}
...
)
{
	...
	if (SearchOperation == SEARCH_OPERATION::ReplaceAndFind)
	{
...
{
	DoSearchOperation(
		SEARCH_OPERATION::ReplaceAndFind,
		WxSearchFlags,
		StringToFind,
		ReplacementString);
}
...
)
{
	...
	while (location != -1)
	{
		...
		SetTargetStart(location + (int)ReplacementString.Length());
		SetTargetEnd(GetTextLength());
...
void CLogoCodeCtrl::ReopenAfterError()
{
	...
	GotoPos((int)g_CharactersSuccessfullyParsedInEditor);
#endif
}
